        ...
        toolTips.forEach((el) => {
            ...
            if (tooltipTargetEl) {
                tooltipTargetEl.onmouseenter = (e) => {
                    let containerBody = document.querySelector(".markdown-body");
                    if (tooltipTarget.startsWith("li.footer-navlist--item")) {
                        containerBody = document.querySelector("header");
                    }

                    let containerBodyOffset = containerBody.getBoundingClientRect();
                    let targetOffset = tooltipTargetEl.getBoundingClientRect();

                    el.classList.toggle("active");
                    // Right the right-hand side of the object
                    let top = targetOffset.top - containerBodyOffset.top + targetOffset.height

                    // Right the right-hand side of the object
                    let left = targetOffset.left - (containerBodyOffset.left - targetOffset.width)
